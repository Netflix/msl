    ...
    var MslControlImpl = util.Class.create({
                            ...
                            builder.getHeader({
                                result: function(requestHeader) {
                                    InterruptibleExecutor(callback, function() {
                                        ...
                                        ctx.getMessageStreamFactory().createOutputStream(ctx, os, MslConstants$DEFAULT_CHARSET, requestHeader, payloadCryptoContext, timeout, {
        ...
        receive: function receive(service, ctx, msgCtx, input, request, timeout, callback) {
            ...
            InterruptibleExecutor(callback, function() {
                ...
                ctx.getMessageStreamFactory().createInputStream(ctx, is, MslConstants$DEFAULT_CHARSET, keyRequestData, cryptoContexts, timeout, {
    ...
    function sendError(service, ctx, debugCtx, recipient, messageId, error, userMessage, output, timeout, callback) {
        MessageBuilder$createErrorResponse(ctx, recipient, messageId, error, userMessage, {
            result: function(errorHeader) {
                ...
                ctx.getMessageStreamFactory().createOutputStream(ctx, output, MslConstants$DEFAULT_CHARSET, errorHeader, null, null, timeout, {
                    result: function(response) {
                        service.setAbort(function() { response.abort(); });
