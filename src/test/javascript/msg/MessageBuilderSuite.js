...
describe("MessageBuilder", function() {
	...
	describe("createResponse", function() {
		...
		it("renew master token with old sequence number", function() {
		    var oldSequenceNumber = 1;
		    var currentSequenceNumber = 3;
            ...
            runs(function() {
                ...
                var renewalWindow = new Date(Date.now() + 10000);
                var expirationWindow = new Date(Date.now() + 20000);
                MasterToken$create(ctx, renewalWindow, expirationWindow, oldSequenceNumber, 1, null, MockPresharedAuthenticationFactory.PSK_ESN, MockPresharedAuthenticationFactory.KPE, MockPresharedAuthenticationFactory.KPH, {
                    result: function(x) { requestMasterToken = x; },
                    error: function(e) { expect(function() { throw e; }).not.toThrow(); }
                });
            });
            ...
            runs(function() {
                ...
                var keyxMasterToken = keyResponseData.masterToken;
                expect(keyxMasterToken.identity).toEqual(requestMasterToken.identity);
                expect(keyxMasterToken.sequenceNumber).toEqual(incrementLong(currentSequenceNumber));
                expect(keyxMasterToken.serialNumber).toEqual(requestMasterToken.serialNumber);
            });
        });
