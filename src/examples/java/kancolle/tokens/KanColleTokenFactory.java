...
public class KanColleTokenFactory implements TokenFactory {
     ...
     */
    @Override
    public boolean acceptNonReplayableId(final MslContext ctx, final MasterToken masterToken, final long nonReplayableId) throws MslException {
        // Fail if the master token is not decrypted.
        if (!masterToken.isDecrypted())
            throw new MslMasterTokenException(MslError.MASTERTOKEN_UNTRUSTED, masterToken);
        ...
        if (nonReplayableId == lastSeenId ||
            nonReplayableId - NON_REPLAYABLE_ID_WINDOW > lastSeenId)
        {
            return false;
        }
     ...
     */
    @Override
    public MasterToken renewMasterToken(final MslContext ctx, final MasterToken masterToken, final SecretKey encryptionKey, final SecretKey hmacKey) throws MslMasterTokenException, MslEncodingException, MslCryptoException {
        // Fail if the master token is not decrypted.
        if (!masterToken.isDecrypted())
            throw new MslMasterTokenException(MslError.MASTERTOKEN_UNTRUSTED, masterToken);
