...
describe("MslEncoderFactory", function() {
    ...
    function createTypedValue(type) {
        ...
        if ("Uint8Array" == type) {
            var b = new byte[random.nextInt(10)];
            random.nextBytes(b);
            return b;
        }
        if ("double" == type)
     ...
     *         expected type.
     */
    function getTypedElement(index, type, ma) throws MslEncoderException {
        if ("boolean" == type)
    ...
    describe("Tokenizer", function() {
    	...
    	beforeEach(function() {
    		if (!params) {
	        	...
	        	var jsonMos;
	        	runs(function() {
		        	var jsonCryptoContext = jsonMessageHeader.getCryptoContext();
    	...
    	parameterize("Parameterized", function data() {
    		return [
    		    ...
    		    [ null, unsupportedMessageData, new MslEncoderException() ],
    		];
    	},
        /**
         ...
         * @param {Uint8Array} messageData encoded message.
         * @param {?Error} exceptionClass expected exception class.
         */
    	function(format, messageData, expectedException) {
	        it("detect tokenizer", function() {
	            ...
	            runs(function() {
			        ...
			        runs(function() {
			            ...
			            // Verify payloads.
			            var payloadCryptoContext = messageHeader.getCryptoContext();
			            function f(i) {
    ...
    describe("Container", function() {
        ...
        it("create object", function() {
            ...
            for (var key : map.keySet()) {
                var expected = map[key];
                var value = mo.opt(key);
                expect(value).toEqual(expected);
                var type = types[key];
                var typedValue = optTypedField(key, type, mo);
                expect(typedValue).toEqual(expected);
            }
            ...
            for (var key in map) {
                ...
                var typedValue = optTypedField(key, randomType, mo);
                ...
                // Numbers may be cross-retrieved...
                    ...
                    ("integer" == randomType ||
                     ...
                     "double" == randomType))
                {
                    expect(numbersEqual(type, expected, typedValue)).toBeTruthy();
                }
                // Double expects NaN...
                ...
                // byte[] expects an empty byte array...
                    ...
                    expect(typeof instanceof Uint8Array).toBeTruthy();
        ...
        it("create array", function() {
            ...
            for (var i = 0; i < list.length; ++i) {
                ...
                expect(value).toEqual(expected);
                var type = types[i];
                var typedValue = getTypedElement(i, type, ma);
                expect(typedValue).toEqual(expected);
            }
            ...
            for (var i = 0; i < list.length; ++i) {
                ...
                expect(value).toEqual(expected);
                var type = types[i];
                var typedValue = optTypedElement(i, type, ma);
                expect(typedValue).toEqual(expected);
            }
            ...
            for (var i = 0; i < list.length; ++i) {
                ...
                var type = types[i];
                var randomType = getRandomType();
                var typedValue = optTypedElement(i, randomType, ma);
            ...
            var s = new textEncoding$toString(b);
    ...
    function(format) {
    	it("encode and parse object", function() {
            ...
            // Encode.
            var encode = encoder.encodeObject(mo, format);
