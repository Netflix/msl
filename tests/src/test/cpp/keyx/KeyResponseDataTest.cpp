 ...
 */
class KeyResponseDataTest : public ::testing::Test
{
public:
    KeyResponseDataTest()
...
protected:
    ...
    shared_ptr<MslObject> MASTER_TOKEN_MO;
};

TEST_F(KeyResponseDataTest, noMasterToken)
{
...
TEST_F(KeyResponseDataTest, noScheme)
{
//    thrown.expect(MslEncodingException.class);
//    thrown.expectMslError(MslError.MSL_PARSE_ERROR);
...
TEST_F(KeyResponseDataTest, noKeydata)
{
//    thrown.expect(MslEncodingException.class);
//    thrown.expectMslError(MslError.MSL_PARSE_ERROR);
...
TEST_F(KeyResponseDataTest, invalidMasterToken)
{
//    thrown.expect(MslEncodingException.class);
//    thrown.expectMslError(MslError.MSL_PARSE_ERROR);

    shared_ptr<ByteArray> encryptionKey = make_shared<ByteArray>();
    shared_ptr<ByteArray> hmacKey = make_shared<ByteArray>();
    shared_ptr<KeyResponseData> response = make_shared<SymmetricWrappedExchange::ResponseData>(MASTER_TOKEN, SymmetricWrappedExchange::KeyId::PSK, encryptionKey, hmacKey);
...
TEST_F(KeyResponseDataTest, unidentifiedScheme)
{
...
TEST_F(KeyResponseDataTest, keyxFactoryNotFound)
{
//    thrown.expect(MslKeyExchangeException.class);
//    thrown.expectMslError(MslError.KEYX_FACTORY_NOT_FOUND);
    ...
    try {
        KeyResponseData::create(ctx1, mo);
        ADD_FAILURE() << "Should have thrown.";
    } catch (const MslKeyExchangeException& e) {
